...
if ($AutoMerge) {
    Write-ColorOutput $Cyan "Auto-merge requested. Creating and merging PR..."
    
    # Check if GitHub CLI is installed
    $ghExists = Get-Command gh -ErrorAction SilentlyContinue
    if (-not $ghExists) {
        Write-ColorOutput $Yellow "GitHub CLI (gh) not found. Creating PR manually..."
        Write-ColorOutput $Yellow "Please visit the PR link to merge manually."
    } else {
        try {
            Write-ColorOutput $Blue "Creating PR with GitHub CLI..."
            
            # Create PR using GitHub CLI
            $prUrl = gh pr create --title "$PRTitle" --body "$PRDescription" --base main --head $BranchName
            
            if ($LASTEXITCODE -eq 0) {
                Write-ColorOutput $Green "PR created successfully: $prUrl"
                
                # Auto-merge the PR
                Write-ColorOutput $Blue "Auto-merging PR..."
                gh pr merge $BranchName --merge --delete-branch
                
                if ($LASTEXITCODE -eq 0) {
                    Write-ColorOutput $Green "PR merged successfully and branch deleted!"
                    Write-ColorOutput $Blue "Switching back to main branch..."
                    git checkout main
                    git pull origin main
                    Write-ColorOutput $Green "Auto-merge workflow completed!"
                } else {
                    Write-ColorOutput $Yellow "PR created but auto-merge failed. You may need to merge manually."
                    Write-ColorOutput $Yellow "PR URL: $prUrl"
                }
            } else {
                Write-ColorOutput $Red "Failed to create PR with GitHub CLI"
                Write-ColorOutput $Yellow "Please visit: https://github.com/lalitnayyar/finally1/compare/$BranchName"
            }
        }
        catch {
            Write-ColorOutput $Red "Error during auto-merge: $($_.Exception.Message)"
            Write-ColorOutput $Yellow "Please visit: https://github.com/lalitnayyar/finally1/compare/$BranchName"
        }
    }
}